 ...
 * <p>
 * This set should be performing better in both terms of memory and speed over the similar
 * implementations (especially for small amount of elements). The adding/removing operations do
 * require an array copy operation. Note that comparator should be as fast as possible, since it's
 * directly influencing the speed of adding/removing the elements.
 * <p>
 ...
 * @author Ivan Senic
 * 
 *         TODO Consider using TreeSet it should be even faster, with a bit higher memory
 *         consumption.
 * 
 * @param <E>
...
public class SortedArraySet<E> extends AbstractCollection<E> implements UpdateableSet<E> {
	...
	/**
	 * {@link AtomicReference} to the array holding the elements in the set.
	 */
	...
	public SortedArraySet(Comparator<E> comparator) {
		if (null == comparator) {
			throw new IllegalArgumentException("Comparator for the DynamicConcurrentSortedArraySet must not be null.");
		}
	 ...
	 * 
	 * * @param array Array to find element in.
	 * 
	 * @param element
	 *            Element to find.
