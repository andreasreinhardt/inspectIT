...
public class ClassHashHelper implements IClassHashHelper, InitializingBean, DisposableBean {
	...
	private AtomicReference<WeakReference<ClassLoader>[]> loaderWeakArrayReference = new AtomicReference<WeakReference<ClassLoader>[]>(new WeakReference[0]);
		...
		public void addClassLoaderReference(WeakReference<ClassLoader> reference) {
			synchronized (this) {
				if (!ArrayUtils.contains(classLoaderReferences, reference)) {
					classLoaderReferences = (WeakReference<ClassLoader>[]) ArrayUtils.add((Object[]) classLoaderReferences, reference);
				}
			}
		}
